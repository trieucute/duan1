    ...
    if(exist_param("user_add")){
            ...
            try {
                ...
                if (isset($_POST['add_user']) !="") {
                    ...
                    }else  if ( preg_match("/^(0|\+84)(\s|\.)?((3[2-9])|(5[689])|(7[06-9])|(8[1-689])|(9[0-46-9]))(\d)(\s|\.)?(\d{3})(\s|\.)?(\d{3})$/",  $_POST['sdt_user']) == false ){
 
                        $error['sdt_user'] = 'Bạn nhập sai số điện thoại';
                      }
                    if( $ho_ten!= "" && $mat_khau != "" && $email != "" && $dia_chi!= "" &&  $sdt!= "" && preg_match("/^(0|\+84)(\s|\.)?((3[2-9])|(5[689])|(7[06-9])|(8[1-689])|(9[0-46-9]))(\d)(\s|\.)?(\d{3})(\s|\.)?(\d{3})$/",  $_POST['sdt_user']) == true &&  $email != khach_hang_exist($email )){
                        if($ext_user !=""){
                            ...
                            $sql_insert_user = "INSERT INTO user (email, mat_khau, vai_tro) VALUES ('$email', '$mat_khau_encryption', '$vai_tro')";
                            $query_insert_user = mysqli_query($connect, $sql_insert_user);
                        }
